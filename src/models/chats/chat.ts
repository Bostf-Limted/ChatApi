...
class ChatModel {
    ...
    private async updateGroup(projectID: number, groupID: number, organization?: string): Promise<void>{
        await this.database.db.group.update({
            where: { projectID, organization, id: groupID },
            data: { lastCommunicated: new Date() }
        });
    }
    
    private async updateChannel(projectID: number, channelID: number, organization?: string): Promise<void>{
        await this.database.db.channel.update({
            where: { projectID, organization, id: channelID },
            data: { lastCommunicated: new Date() }
        });
    }

    async create(data: { projectID: number, token: string, jwtAccessKey: string, message: string, receiverID?: string, groupID?: number, organization?: string }): Promise<Chat | GroupChat | undefined>{
        try{
            ...
            if(data.groupID){
                ...
                await this.updateGroup(data.projectID, data.groupID, data.organization);
            ...
            if(data.receiverID){
                let channel = await this.database.db.channel.findFirst({
                    where: { 
                        ...
                        organization: data.organization, projectID: data.projectID
                    }
                });

                if(!channel){
                    channel = await this.database.db.channel.create({ 
                        data: { projectID: data.projectID, userOneID: user.id, userTwoID: data.receiverID, organization: data.organization },
                    });
                }
                ...
                await this.updateChannel(data.projectID, channel.id, data.organization);

                await this.database.db.notification.create({
                    data: { 
                        senderID: user.id, recieverID: data.receiverID, projectID: data.projectID, organization: data.organization,
    ...
    async reply(data: { projectID: number, token: string, jwtAccessKey: string, chat: { message: string, chatID: number, channelID?: number, groupID?: number}, organization?: string }): Promise<Chat | GroupChat | undefined>{
        try{
            const { user } = jwt.verify(data.token, data.jwtAccessKey ) as JwtPayload;
            ...
            if(data.chat.groupID){
                ...
                await this.updateGroup(data.projectID, data.chat.groupID, data.organization);
                ...
                members.forEach(async  (member)=>{
                    if(user.id !== member.userID){
                        await this.database.db.notification.create({
                            data: { 
                                groupID: data.chat.groupID, senderID: user.id, recieverID: member.userID, projectID: data.projectID, organization: data.organization,
            ...
            if(data.chat.channelID){
                ...
                await this.updateChannel(data.projectID, data.chat.channelID, data.organization);

                await this.database.db.notification.create({
                    data: { 
                        senderID: user.id, recieverID: chat.reference?.sender.name!, projectID: data.projectID , organization: data.organization,
    ...
    async update(data: { projectID: number, token: string, jwtAccessKey: string, message: string, chatID: number, channelID?: number, groupID?: number, organization?: string }): Promise<Chat | GroupChat | undefined>{
        try{
            const { user } = jwt.verify(data.token, data.jwtAccessKey ) as JwtPayload;
    ...
    async seen(data: { projectID: number, token: string, jwtAccessKey: string, chatID: number, channelID?: number, groupID?: number, organization?: string }): Promise<Chat | Delivered | undefined>{
        try{
