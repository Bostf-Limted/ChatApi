...
class UserModel{
    ...
    async create(data: { token: string, platform: string, organization?: string }): Promise<User | undefined>{
        try{
            ...
            const init = await this.database.db.user.upsert({
                where: { id: user.id, platform: data.platform, organization: data.organization }, 
    ...
    async get(data: { token: string, platform: string, organization?: string }): Promise<{ user: User, chats: Array<Group | Channel>, notifications: Notification[] } | undefined>{
        try{
            const { user } = jwt.verify(data.token, process.env.PRIVATE_ACCESS_TOKEN || "bobby_access" ) as JwtPayload;

            const notifications = await this.database.db.notification.findMany(
                { where: { recieverID: user.id, platform: data.platform, organization: data.organization, },
                include: { sender: true, group: true },
                orderBy: { created: "desc" }
            });

            const channels = await this.database.db.channel.findMany({ 
                where: { OR: [ { userOneID: user.id, }, { userTwoID: user.id } ], platform: data.platform, organization: data.organization },
                include: { chats: { include: {  sender: true }, orderBy: { createdAt:  "desc" } } },
            });

            const memebers = await this.database.db.member.findMany({
                where: { userID: user.id, group: { platform: data.platform, organization: data.organization } },
            });
